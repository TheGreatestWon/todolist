...
**개발 기간:** 3일
**현재 상태:** 백엔드/프론트엔드 구현 완료, 통합 테스트 진행 중
...
### 6.2 프론트엔드 ✅
- **프레임워크:** React 19.2.4
- **언어:** TypeScript 4.9.5 (strict mode)
- **상태 관리:** React Hooks (Context API) - AuthContext 구현 완료
- **HTTP 클라이언트:** Fetch API (axios 사용 금지)
- **라우팅:** React Router 7.13.0
- **테스팅:** Jest + React Testing Library 16.3.2
- **스타일링:** CSS (예정)

### 6.3 백엔드 ✅
- **런타임:** Node.js
- **프레임워크:** Express
- **데이터베이스 클라이언트:** pg (PostgreSQL 전용) - 구현 완료
- **인증:** JWT (JSON Web Token) - jsonwebtoken 라이브러리
- **비밀번호 암호화:** bcrypt - 구현 완료
- **아키텍처:** 계층형 (Controller → Service → Repository)

**중요:** Prisma 사용 금지 - pg 라이브러리 직접 사용 (준수 완료)

### 6.4 데이터베이스 ✅
- **DBMS:** PostgreSQL 17
- **스키마 관리:** SQL 스크립트 직접 작성 (schema.sql)
- **스키마 구성:** users, todos 테이블 생성 완료
- **인덱스:** 5개 인덱스 최적화 완료
- **트리거:** updated_at 자동 갱신 트리거 구현 완료
...
- [x] 데이터베이스 스키마 생성 (users, todos 테이블)
- [x] Express 서버 기본 구조 설정

**오후 (4시간):** ✅ 완료
- [x] 회원가입 API 개발 (POST /api/auth/register)
- [x] 로그인 API 개발 (POST /api/auth/login)
- [x] JWT 인증 미들웨어 구현
- [x] 할 일 CRUD API 개발
  - [x] GET /api/todos
  - [x] POST /api/todos
...
**오전 (4시간):** ✅ 완료
- [x] React + TypeScript 프로젝트 초기 설정
- [x] 라우팅 설정 (React Router)
- [x] 회원가입 페이지 개발
- [x] 로그인 페이지 개발
- [x] 인증 Context 및 상태 관리 구현 (AuthContext)
- [x] 로컬스토리지에 JWT 토큰 저장

**오후 (4시간):** 🔄 진행 중
- [x] 할 일 목록 페이지 레이아웃 개발
- [x] 할 일 추가 폼 구현
- [x] 할 일 목록 조회 및 표시
- [x] 할 일 수정 기능 구현
- [x] 할 일 삭제 기능 구현
- [x] 완료 처리 체크박스 구현
- [x] API 연동 (Fetch API 사용)
...
**오전 (4시간):** 🔄 진행 중
- [x] 기한 경과 자동 판단 로직 구현 (isOverdue 함수)
- [x] 할 일 분류 및 정렬 로직 구현 (TodoStatus enum)
- [ ] 완료/미완료 섹션 분리 표시
- [ ] UI 스타일링 및 색상 적용
- [ ] 반응형 디자인 구현
- [ ] 모바일 화면 최적화
...
**최종 점검:**
- [x] 모든 API 엔드포인트 정상 작동 확인 (백엔드 테스트 완료)
- [x] 인증 및 권한 검증 확인 (JWT 미들웨어 구현)
- [ ] UI/UX 최종 검토
- [ ] 성능 테스트 (로딩 시간 확인)
...
- [x] **AC-001-01:** 사용자는 유효한 이메일 형식을 입력하여 회원 가입할 수 있다
- [ ] **AC-001-02:** 비밀번호는 최소 8자 이상이어야 하며, 8자 미만 시 가입이 차단된다 (UI 검증 필요)
- [x] **AC-001-03:** 중복된 이메일로는 가입할 수 없으며, "이미 사용 중인 이메일입니다" 메시지가 표시된다
- [ ] **AC-001-04:** 잘못된 이메일 형식 입력 시 "유효한 이메일 주소를 입력하세요" 메시지가 표시된다 (UI 검증 필요)
- [x] **AC-001-05:** 가입 성공 시 자동으로 로그인 페이지로 리다이렉트된다 (프론트엔드 구현 완료)
- [ ] **AC-001-06:** 이메일 또는 비밀번호 필드가 비어있으면 가입 버튼이 비활성화된다 (UI 구현 필요)
- [x] **AC-001-07:** 비밀번호는 데이터베이스에 bcrypt로 암호화되어 저장된다 (백엔드 구현 완료)

---

### 14.2 로그인 (FR-002)

- [x] **AC-002-01:** 사용자는 등록된 이메일과 비밀번호로 로그인할 수 있다 (백엔드 구현 완료)
- [x] **AC-002-02:** 올바른 자격증명 입력 시 JWT 토큰이 발급된다 (백엔드 구현 완료)
- [x] **AC-002-03:** 로그인 성공 시 할 일 목록 페이지(/todos)로 자동 이동한다 (프론트엔드 구현 완료)
- [x] **AC-002-04:** 잘못된 이메일 입력 시 "이메일 또는 비밀번호가 올바르지 않습니다" 메시지가 표시된다
- [x] **AC-002-05:** 잘못된 비밀번호 입력 시 "이메일 또는 비밀번호가 올바르지 않습니다" 메시지가 표시된다
- [x] **AC-002-06:** 로그인 상태는 브라우저를 닫아도 유지된다 (로컬스토리지 JWT 저장)
- [x] **AC-002-07:** 로그아웃 버튼을 클릭하면 로그인 페이지로 이동하고 JWT 토큰이 삭제된다
- [x] **AC-002-08:** 로그인하지 않은 사용자가 /todos 페이지 접근 시 로그인 페이지로 리다이렉트된다 (ProtectedRoute)
- [ ] **AC-002-09:** 이미 로그인한 사용자가 로그인 페이지 접근 시 할 일 목록 페이지로 리다이렉트된다 (구현 필요)

---

### 14.3 할 일 생성 (FR-003)

- [x] **AC-003-01:** 로그인한 사용자만 할 일을 생성할 수 있다 (authMiddleware)
- [x] **AC-003-02:** 제목은 필수 입력 항목이며, 비어있으면 생성이 차단된다 (백엔드 검증)
- [x] **AC-003-03:** 제목은 최대 200자까지 입력 가능하다 (DB 제약)
- [ ] **AC-003-04:** 설명은 선택 입력 항목이며 최대 1000자까지 입력 가능하다 (UI 구현 필요)
- [ ] **AC-003-05:** 마감일은 선택 항목이며 날짜 선택기(date picker)로 선택한다 (UI 구현 필요)
- [ ] **AC-003-06:** 마감일은 오늘 또는 미래 날짜만 선택 가능하다 (과거 날짜 선택 불가) (UI 검증 필요)
- [x] **AC-003-07:** 생성된 할 일은 자동으로 "미완료(Incomplete)" 상태로 설정된다 (DB DEFAULT)
- [x] **AC-003-08:** 생성 시점(created_at)이 자동으로 기록된다 (DB DEFAULT)
- [x] **AC-003-09:** 생성된 할 일은 즉시 목록에 표시된다 (프론트엔드 구현 완료)
- [ ] **AC-003-10:** 생성 성공 후 입력 폼이 초기화된다 (제목, 설명, 마감일 비워짐) (UI 구현 필요)
- [x] **AC-003-11:** 생성된 할 일은 현재 로그인한 사용자에게 소유권이 할당된다 (user_id 자동 설정)
...
- [x] **AC-NFR-005:** 비밀번호는 bcrypt로 암호화되어 저장되어야 한다 (백엔드 구현 완료)
- [x] **AC-NFR-006:** 평문 비밀번호는 데이터베이스에 절대 저장되지 않는다 (password_hash 사용)
- [x] **AC-NFR-007:** JWT Secret은 환경 변수로 관리되며 코드에 하드코딩되지 않는다 (process.env.JWT_SECRET)
- [x] **AC-NFR-008:** 모든 할 일 API 요청은 유효한 JWT 토큰을 포함해야 한다 (authMiddleware)
- [x] **AC-NFR-009:** 토큰이 없거나 유효하지 않은 요청은 401 Unauthorized로 거부된다 (authMiddleware)
- [x] **AC-NFR-010:** SQL Injection 공격이 방어된다 (파라미터화된 쿼리 사용 - pg)
- [ ] **AC-NFR-011:** XSS 공격이 방어된다 (입력값 이스케이프 처리) (프론트엔드 검증 필요)
- [ ] **AC-NFR-012:** 프로덕션 환경에서 HTTPS가 사용된다 (Vercel 배포 시 자동 적용)
- [x] **AC-NFR-013:** 사용자는 자신의 데이터만 접근할 수 있다 (100% 격리) (user_id 검증)
- [x] **AC-NFR-014:** 다른 사용자의 할 일 접근 시도는 403 Forbidden으로 차단된다 (소유권 검증)
...
- [x] **AC-NFR-029:** 사용자 삭제 시 해당 사용자의 모든 할 일이 CASCADE로 함께 삭제된다 (ON DELETE CASCADE)
- [x] **AC-NFR-030:** 제목 없는 할 일은 생성될 수 없다 (데이터베이스 제약 - NOT NULL)
- [x] **AC-NFR-031:** 소유자 없는 할 일은 생성될 수 없다 (데이터베이스 제약 - NOT NULL)
- [x] **AC-NFR-032:** 이메일 중복이 데이터베이스 레벨에서 방지된다 (UNIQUE 제약)
...
| 2.0  | 2026-02-13 | Claude Code | 구현 완료 후 갱신: 기술 스택 검증, 일정 체크리스트 업데이트, 인수 기준 검증 상태 반영 |

---

## 19. 변경 이력 상세 (Change Log)

### 버전 2.0 (2026-02-13) - 구현 완료 후 갱신

**갱신 사유**: 백엔드 및 프론트엔드 구현 완료 후 실제 진행 상황 반영

**주요 변경 사항**:

1. **문서 메타데이터 업데이트**
   - 문서 버전: 1.0 → 2.0
   - 최종 갱신일 추가: 2026-02-13
   - 현재 상태 추가: 백엔드/프론트엔드 구현 완료

2. **기술 스택 검증 및 상세화** (6장)
   - 프론트엔드 버전 명시: React 19.2.4, TypeScript 4.9.5
   - HTTP 클라이언트 확정: Fetch API (axios 금지 준수)
   - 백엔드 아키텍처 추가: 계층형 (Controller → Service → Repository)
   - 데이터베이스 구현 상세 추가: 인덱스 5개, 트리거 구현 완료

3. **일정 및 마일스톤 진행 상황 업데이트** (10장)
   - Day 1 (백엔드): ✅ 완료 표시
     - 모든 API 엔드포인트 구현 완료
     - JWT 인증 미들웨어 구현 완료
   - Day 2 (프론트엔드): ✅ 오전 완료, 🔄 오후 진행 중
     - 인증 Context 구현 완료
     - Todo 컴포넌트 구현 완료
     - API 연동 완료
   - Day 3 (통합/배포): 🔄 진행 중
...
4. **인수 기준 검증 상태 반영** (14장)
   - **회원가입 (AC-001)**: 3/7 완료
     - bcrypt 암호화 구현 완료
     - 중복 이메일 검증 완료
   - **로그인 (AC-002)**: 8/9 완료
     - JWT 토큰 발급 완료
     - ProtectedRoute 구현 완료
   - **할 일 생성 (AC-003)**: 7/11 완료
     - authMiddleware 인증 완료
     - DB 제약조건 적용 완료
   - **보안 (AC-NFR-005~014)**: 8/10 완료
     - bcrypt, JWT 구현 완료
     - SQL Injection 방어 완료
     - 소유권 검증 완료
   - **데이터 무결성 (AC-NFR-029~032)**: 4/4 완료
     - ON DELETE CASCADE 구현
     - UNIQUE, NOT NULL 제약 적용

5. **구현 완료 항목 체크 표시**
   - ✅ 표시: 구현 및 검증 완료
   - 🔄 표시: 진행 중
   - [ ] 표시: 미구현 (UI 스타일링, 통합 테스트 등)
